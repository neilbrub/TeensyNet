int mapInputsToNote() {
  ...
  int val1 = inputButtonValues[5];
  int val2 = inputButtonValues[4];
  int val3 = inputButtonValues[3];
  int val4 = inputButtonValues[2];
  int val5 = inputButtonValues[1];
  int val6 = inputButtonValues[0];

  // Invalid symbol -1  
  int note = -1;

  // EVI Fingering, with low F open
  if (!val1 && val2 && val3 && !val4 && !val5 && !val6) note = 77; //F5
  else if (!val1 && val2 && val3 && !val4 && val5 && !val6) note = 76; //E5
